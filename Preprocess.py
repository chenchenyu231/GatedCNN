...
def sample_data(dataset_A, dataset_B,frames_per_sample=128,perm=[1,0],s_type='parallel'):
    ...
    if dim == 2:
        ...
        feature_dim = np.mean([data.shape[feature_index] for idx,data in enumerate(dataset_A)])
        assert feature_dim.is_integer() 
        feature_dim=int(feature_dim)
        train_data_A = np.empty([total_sample,feature_dim,frames_per_sample])
        train_data_B = np.empty([total_sample,feature_dim,frames_per_sample])
        ...
        for idx,data in enumerate(dataset_A):
            height_dim=np.mean(data.shape[height_index])
            width_dim=np.mean(data.shape[width_index])
            channel_dim=np.mean(data.shape[channel_index])
        
        assert height_dim.is_integer() and width_dim.is_integer() and channel_dim.is_integer()
        height_dim=int(height_dim)
        width_dim=int(width_dim)
        height_dim=int(channel_dim)
        
        train_data_A = np.empty([total_sample,height_dim,width_dim,channel_dim])
        train_data_B = np.empty([total_sample,height_dim,width_dim,channel_dim])
    ...
    for idx,list_idx in tqdm(enumerate(zip(train_data_A_idx[:total_sample], train_data_B_idx[:total_sample])),desc='Data Sampling'):
        idx_A=list_idx[0]
        idx_B=list_idx[1]
        data_A=dataset_A[idx_A]
        frames_A_total = data_A.shape[1]
