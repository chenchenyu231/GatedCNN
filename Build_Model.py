...
class Generator(object):
    ...
    def __init__(self, num_features, num_frames, generator = 'generator_gatedcnn',discriminator = None, mode = 'train', ini_weights = None, log_dir = './log'):
        ...
        if generator in ['generator_gatedcnn','gatedcnn','Gatedcnn','GatedCnn','GatedCNN']:
            ...
            self.input_shape = [None, num_features] # [batch_size*num_frames, num_features]
        else:
...
#        self.generator_loss_A2B = LossFunc().mean_square_error(y_true = self.input_B_real, y_pred = self.generation_A2B)
    ...
    def total(self):
        sum1=np.sum([np.product([xi.value for xi in x.get_shape()]) for x in tf.all_variables()])
        return sum1
if __name__ == '__main__':
    
    model = Generator(num_features = 24,num_frames = 128, ini_weights = './model/gatedcnn_wang/gatedcnn_wang.ckpt', generator = 'gatedcnn')
#    model = Generator(num_features = 24,num_frames = 128, ini_weights = './model/20190918/debug.ckpt', generator = 'dnn')
